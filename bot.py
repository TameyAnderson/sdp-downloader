...
_user_sems = {}          # user_id -> Semaphore (parallel downloads per user)
_chat_last_send = {}     # chat_id -> monotonic timestamp of the last send
_chat_send_lock = {}     # chat_id -> asyncio.Lock


def _user_gate(user_id):
    limit = max(1, tunable("max_per_user"))
    sem, made_for = _user_sems.get(user_id, (None, None))
    if sem is None or made_for != limit:
        sem = asyncio.Semaphore(limit)      # limit changed -> fresh gate
        _user_sems[user_id] = (sem, limit)
    return sem
...
class FloodMiddleware:
    ...
    async def __call__(self, make_request, bot, method):
        ...
        if chat_id is not None and interval > 0:
...
T = {
    "uk": {
        ...
        "ck_expiring": ("⏳ Cookies Instagram спливають приблизно через {days} дн.\n"
                        "Онови сесію: зайди в акаунт, експортуй cookies.txt "
                        "і надішли файл сюди — я збережу."),
        "ck_expired": ("🔴 Cookies Instagram протерміновані — приватні пости знову недоступні.\n"
    ...
    "en": {
        ...
        "ck_expiring": ("⏳ Instagram cookies expire in about {days} days.\n"
                        "Refresh the session: log in, export cookies.txt "
                        "and send the file here — I will store it."),
        "ck_expired": ("🔴 Instagram cookies have expired — private posts are unavailable again.\n"
...
def db_init():
    ...
    try:
        ...
        for _col in ("size INTEGER", "thumb_id TEXT"):
            try:
                con.execute("ALTER TABLE events ADD COLUMN " + _col)
            except Exception:  # noqa: BLE001
                pass
        ...
        # One-time move to the simple model: a whitelist of chats, no levels.
        row = con.execute(
            "SELECT value FROM settings WHERE key='whitelist'").fetchone()
        if row is None:
            old = con.execute(
                "SELECT value FROM settings WHERE key='access_mode'").fetchone()
        ...
        con.commit()
        con.close()
        logger.info("Stats DB ready: %s", STATS_DB)
    except Exception:  # noqa: BLE001
...
async def send_media(bot, message, items, source, strict=False):
    ...
    if not items:
        return False
    tmp = Path(tempfile.gettempdir()) / ("vbot_" + uuid.uuid4().hex)
    ...
    try:
        async with _user_gate(uid):
            ...
            for attempt in range(job_retries + 1):
                try:
                    status, source = await _do_process(
                        bot, message, url, is_private, want_audio, plat, source
                    )
                    job["source"] = source
                    break
...
async def main():
    ...
    asyncio.create_task(update_checker_loop(bot))

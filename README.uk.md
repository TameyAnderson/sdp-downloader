...
- **All-in** — один тумблер, і бот бере будь-який сайт із тих 1500+, що підтримує `yt-dlp`.
  Посилання перевіряються локально за обробниками сайтів установленої версії
  `yt-dlp`, без універсального режиму «спробувати будь-яку сторінку».
  Невідомі посилання бот мовчки ігнорує, також у змішаних повідомленнях і підписах.
  Прямі посилання на медіафайли з невідомих сайтів теж ігноруються. Перемикачі
  сервісів зберігають дію; список сайтів оновлюється разом із yt-dlp після перезапуску.
...
Офіційні компоненти `@material/web`: м’ятно-бірюзова палітра Material 3, делікатні
градієнтні акценти, перемикачі, окреслені поля. Світла, темна й автоматична тема за Telegram
(або системною темою у браузері). Вибір теми зберігається на пристрої.
На вузьких екранах навігація внизу, на широких — збоку.
Прев’ю, завантаження, історія, статистика й адміністративні налаштування збережені.

Docker автоматично збирає інтерфейс окремим етапом із Node.js; у робочому образі
бота Node немає. Компоненти віддаються локально через `/assets/material.js`, без CDN.
Для запуску без Docker потрібен Node.js 22+: виконай `npm ci`, потім `npm run build`
перед запуском бота. Папка `static/` має бути поруч з `index.html`, зокрема при зміні
`INDEX_HTML_PATH`. Після оновлення залежностей інтерфейсу повтори збірку.
Для локального перегляду після збірки просто відкрий `index.html` подвійним кліком
у браузері. Теми й навігація працюють без бота; дані та збереження налаштувань
не підключені. Не переміщуй `index.html` окремо від папки `static/`.
Material Web перебуває в режимі підтримки: це вебінтерфейс M3 з акцентами за мотивами
Expressive, а не повний набір Android Expressive чи Jetpack Compose.
...
Дата додавання cookies зберігається в постійній базі, а не в часі зміни файлу.
Перезапуск, редеплой і повторне надсилання однакового вмісту її не скидають.
Для старих файлів без метаданих дата невідома. Залишок часу — оцінка за датами
сесійних cookies, а не підтвердження працездатності входу. Кожен процес yt-dlp
отримує окрему тимчасову копію cookies та не переписує оригінал.
Видалення cookies також очищає їхній експорт для Cobalt.
...
Для Stories потрібна Instagram-cookie `sessionid`, а сторіс має бути доступною
цьому акаунту. Якщо завантаження не працює, спочатку відкрий сторіс у
залогіненому браузері, з якого експортуєш cookies, потім онови файл у боті.
Майбутня дата закінчення cookies не гарантує, що Instagram досі приймає сесію.
Cookies з префіксом `#HttpOnly_` підтримуються. Stories обробляє безпосередньо
yt-dlp: відмова доступу не запускає повтори на нижчій якості чи через Cobalt.
...
Для вкладень із чату (зокрема cookies.txt) сервіс `telegram-bot-api` має працювати
з `TELEGRAM_LOCAL: "1"` і ділити том `tgapi-data` з `video-bot` за шляхом
`/var/lib/telegram-bot-api` (у бота — лише читання). Це вже задано у Full compose.
При оновленні старого стека перествори **обидва** сервіси з новою конфігурацією:
перезапуск лише бота не застосує нові змінні сервера. Томи видаляти не потрібно.

Образ бота працює під UID **101**, як і комплектний образ Telegram API.
Telegram створює приватні файли (0600) та каталоги (0750), тому попередній UID
бота 10001 не міг читати вкладення навіть із правильно підключеним томом.
Для виправлення завантаж або збери новий **образ бота** та перествори `video-bot`:
звичайний перезапуск старого образу не допоможе. При запуску права власності
на `/data` та том експорту `/cookies` оновляться автоматично.
Том Telegram залишається лише для читання;
`chmod 777` не потрібен. Для іншого образу Telegram API UID власника файлів
потрібно узгодити окремо.

Відносний шлях `documents/file_*.txt` і HTTP 404 зазвичай означають, що локальний
режим не ввімкнено; недоступний абсолютний шлях — проблему спільного тому або прав.
